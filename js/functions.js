...
define([
    ...
    'addons/wpak-addon-wp-image-trad/js/imageMapResizer',
    'theme/js/jquery.fitvids'
    ], function($,App,Storage,TemplateTags,Config,Moment,Velocity,ImageMapResizer) {

    $("#app-layout").on("click",".mobile-image-map-fix", function(event){
...
console.log('$map.length',$map.length);
        if ( $map.length ) {
            var map_id = $map.data('id');
            var map_fix_id = "mobile-image-map-fix-"+map_id;
            console.log('.mobile-image-map-fix.length',$("#"+map_fix_id).length);
            if( !$("#"+map_fix_id).length ) {
                //automatically cover any image maps we find with a div
                console.log('Add div');
                $("map").after('<div class="mobile-image-map-fix" id="'+map_fix_id+'" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"/>');
            }
        }
